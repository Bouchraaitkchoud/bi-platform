...
export default function DatasetExplorePage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const previewData = await DatasetService.getDatasetPreview(datasetId, pageSize);
      setPreview(previewData as any);
      setError('');

      // Try to fetch statistics but don't fail if it errors
      try {
        const stats = await DatasetService.getColumnStatistics(datasetId, ['*']);
        setStatistics((stats || {}) as any);
      } catch (statErr) {
        console.warn('Failed to load statistics:', statErr);
        setStatistics({});
      }
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f6f7' }}>
      ...
      >
        ...
        {/* Stats Bar */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px', fontSize: '13px', marginBottom: '16px' }}>
          <div>
        ...
        {/* Workflow Progress */}
        <div style={{ display: 'flex', gap: '8px', fontSize: '11px', opacity: 0.9, flexWrap: 'wrap' }}>
          <span style={{ padding: '2px 8px', backgroundColor: 'rgba(45, 134, 89, 0.2)', color: 'white', fontWeight: 'bold', borderRadius: '4px' }}>✓ Import</span>
          <span style={{ padding: '2px 8px', backgroundColor: 'rgba(255,255,255,0.8)', color: '#2d8659', fontWeight: 'bold', borderRadius: '4px' }}>● Explore</span>
      ...
      <div style={{ padding: '32px', maxWidth: '1400px', margin: '0 auto' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <button
              onClick={() => router.push(`/clean?datasetId=${datasetId}`)}
              style={{
